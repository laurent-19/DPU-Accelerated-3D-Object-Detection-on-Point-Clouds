# -----------------------------------------------------------------------------------
# Description: 
# DPU-accelerated 3D object detection on point clouds (2 side back-front view).
...
def get_system_stats():
    ...
    try:
        ...
        # Check if valid output even with non-zero return code
...
def init_csv_file(csv_filename):
    ...
    headers = [
        'Frame', 'Front_DL_FPS', 'Back_DL_FPS', 'Power_mW',
...
def write_stats_to_csv(csv_filename, frame_id, front_fps, back_fps, system_stats, headers):
    """Write frame statistics to CSV file"""
    row = {
        'Frame': frame_id,
        'Front_DL_FPS': front_fps,
        'Back_DL_FPS': back_fps
    }
...
if __name__ == '__main__':
    ...
    # Initialize CSV file for benchmarking data
    csv_filename = os.path.join(configs.results_dir, f'benchmark_stats_{configs.foldername}.csv')
    headers = init_csv_file(csv_filename)
    print(f'Benchmark data will be saved to: {csv_filename}')
    ...
    for sample_idx in range(len(demo_dataset)):
        ...
        front_detections, front_bevmap, front_fps, front_inference_time = do_detect(dpu, shapeIn, image, input_data, output_data, configs, front_bevmap, is_front=True)
        back_detections, back_bevmap, back_fps, back_inference_time = do_detect(dpu, shapeIn, image, input_data, output_data, configs, back_bevmap, is_front=False)

        # Write benchmark data to CSV
        write_stats_to_csv(csv_filename, sample_idx, front_fps, back_fps, system_stats, headers)
        
        #print(f'Frame {sample_idx}: Front FPS={front_fps:.2f}, Back FPS={back_fps:.2f}')
        #if system_stats:
        #    print(f'  Power: {system_stats.get("SOM_total_power_mW", "N/A")}mW, '
        #          f'Temp: {system_stats.get("PL_temp_C", "N/A")}°C')
